...
4. **Emit-time validation (§3.7 reverse guard).** Before emitting any recipe: write the recipe JSON to a temp file and run `node validate-recipe-roles.js <recipe.json>`. Exit 1 means at least one entry asserts a role not in that gear's `roles[]` (or `sides[sideId].roles[]` when a side is specified) — **regenerate, never emit.** This check runs the validator shipped inside the beta HTML, so it cannot drift from the app.
   - **Exact chain-entry keys (the validator reads these literally):** each entry in `pedalChain` / `effectsLoopChain` / `amps` is `{ gearId, role, side? }` — `gearId` (required, must exist in `gear-inventory.js`), `role` (the asserted role, checked against the gear's `roles[]`), `side` (optional `sideId` for dual-side units). `substitutionAudit[]` rows carry `method` (from the §3.5 enumeration). **Spell `role` exactly** — see the limitation below.
   - **KNOWN LIMITATION until Phase 4 (G-VAL-1, ROADMAP).** The guard validates role *values* but does not yet reject *unrecognized keys*: a mistyped key (e.g. `roleAssertion`) leaves the entry with no `role`, so its assertion is silently skipped and the recipe passes VALID. Until Phase 4 hardens this, a green exit 0 is **not** proof your keys are spelled right — eyeball every entry for the literal key `role`.
