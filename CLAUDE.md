...
**Gear tab — Phase 3.** Phase 1 (gear-inventory.js authoring) shipped 2026-05-03. Phase 2 (`buildRoleIndex()` runtime + role tag audit) shipped 2026-05-17 — `<script src="gear-inventory.js">` loaded after dashboard-data.js, `window.ROLE_INDEX` built at app load with `resolveRole()`/`roleIndexGaps()` helpers (Grep banner `GEAR ROLE INDEX (Gear Tab Phase 2`), 81 roles verified, `ROLE_INDEX['univibe-modulation'] = ['mxr-m68-univibe']` live. Phase 3 is the Tone Engineer agent update (role-index lookup + §3.7 reverse-guard validator) + KWS recipe rerun. Implementation phases are listed in Section 6 of `GEAR_TAB_DESIGN.md`; Phase 4.5 is the minimum-viable stopping point. Read Sections 0-3 before any code work — particularly Section 3.7 (schema-level role assertion validation) and Section 3.4 (notation fidelity per setting).
...
- **Verify changes — automated behavior floor + manual checks:**
  - *Behavior floor (automated)* — open `TONE__BETA_v*.html?selftest` (green banner = pass) or run `runSelfTests()` in the console; 23 assertions over the pure-function core (CHORD_REGISTRY SSOT, scale rotation, `identifyChord`, `computeNumeral`, role index). Must be green before commit — this is the `/session-close` gate. Grep banner `SELF-TEST HARNESS`; add an assertion when you add engine behavior.
