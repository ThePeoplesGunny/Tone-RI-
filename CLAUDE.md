...
**Gear tab — Phase 4.** Phases 1–3 shipped: Phase 1 (gear-inventory.js) 2026-05-03; Phase 2 (`buildRoleIndex()` runtime, Grep banner `GEAR ROLE INDEX`) 2026-05-17; Phase 3 (Tone Engineer agent recipe-mode protocol + §3.7 reverse guard) 2026-06-09. The §3.7 validator is `validateRecipeRoles()` in the HTML (walks `pedalChain`/`effectsLoopChain`/`amps`, rejects role assertions not in the gear's `roles[]`/`sides[].roles[]`; also rejects `substitutionAudit[].method` values outside the §3.5 enumeration) with `validate-recipe-roles.js` as the agent-facing CLI. Role vocabulary normalized in the Phase 3 review (81 → 74 roles; description-only tags deleted, `srv-boost`/`kws-boost` → `*-substitute` pattern). KWS Voodoo Child 1997 rerun verified: Univibe DIRECT MATCH (`mxr-m68-univibe`), Phase 90 assertion REJECTED at validation. Phase 4 is `TONE_RECIPES` schema + marker pair in the HTML + first two hand-authored recipes (KWS While We Cry 1995, KWS Voodoo Child 1997) + `'be-here-now-rhythm'` prototype migration. Implementation phases are listed in Section 6 of `GEAR_TAB_DESIGN.md`; Phase 4.5 is the minimum-viable stopping point. Read Sections 0-3 before any code work — particularly Section 3.7 (schema-level role assertion validation) and Section 3.4 (notation fidelity per setting).
...
- **Verify changes — automated behavior floor + manual checks:**
  - *Behavior floor (automated)* — open `TONE__BETA_v*.html?selftest` (green banner = pass) or run `runSelfTests()` in the console; 29 assertions over the pure-function core (CHORD_REGISTRY SSOT, scale rotation, `identifyChord`, `computeNumeral`, role index, §3.7/§3.5 recipe validator). Must be green before commit — this is the `/session-close` gate. Grep banner `SELF-TEST HARNESS`; add an assertion when you add engine behavior. Headless run (no manual browser step): `& "C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe" --headless=new --disable-gpu --dump-dom "file:///C:/claude/TONE%20RI/TONE__BETA_v4_4_.html?selftest"` and grep the output for `TONE self-tests: ... all green`.
...
- `gear-inventory.js` — `GEAR_INVENTORY` constant (Gear tab Phase 1). Hand-maintained from `gear.txt`; covers 38 in-scope guitar-tone items across 10 categories (guitars / amps / cabinets / ampInABox / drives / modulation / timeBased / wahs / utility / modeler) with structured controls, switches, sides, and role tags per `GEAR_TAB_DESIGN.md` §3.1. Roles[] feed the role-index lookup and the §3.7 schema validation guard. Hand-edit the file directly when gear changes; no in-app authoring path. **LD#9: each pedal carries a `power` block + per-item `specProvenance` stamp; `default` values are author-inferred, never spec. Populated 2026-05-18 across all 27 powered entries (26 pedals + MONDO): **all 27 `confirmed`, 0 `unconfirmed` (100%, completed 2026-05-18 via owner attestation).** **Provenance hierarchy (P2): owner-attested > manufacturer manual > retailer/secondary > schema. Manual is source of truth over secondary; owner attestation overrides even the manual and corrects wrong data on evidence (P14).** 2026-05-18 owner attestations: `line-6-helix` corrected (internal PSU + standard IEC AC mains — prior DC-3-brick research was wrong, the HX Stomp/LT pattern), `king-tone-duellist` confirmed (9–18V, 2.1mm center-neg, ~10mA, ≥100mA isolated rec.; voltage is a deliberate tone control — see entry notes), `evh-5150-od` confirmed (9V DC or 9V battery; mA still secondary). Flashback 2 confirmed from verbatim manual (unit attested as Flashback 2; entry renamed); MC404 boost corrected +15dB→+20dB per manual. Residual unconfirmed items tracked in ROADMAP "Tracked follow-ups from Gear tab v0.2".**
...
- `validate-recipe-roles.js` — Phase 3 CLI for the §3.7 reverse guard. Extracts the SHIPPED `validateRecipeRoles()` from the current beta HTML (banner-string slice — no duplicated logic to drift) and runs it against gear-inventory.js. `node validate-recipe-roles.js <recipe.json>` (exit 0/1), `--index` dumps ROLE_INDEX, `--roles <gearId>` lists one item's roles. The Tone Engineer agent must run the recipe form before emitting any recipe.
...
| 8 | `buildRoleIndex()` emits structurally-derived `quality` only (`direct` / `amp-in-a-box` / `capture` / `fallback`) | GEAR_TAB_DESIGN §3.6's illustrative result shows judgment grades (`closest-analog`) and per-switch voicing (`voicing:'Brown'`) that are NOT derivable from `roles[]`. Structural index = structural truth; substitution-method judgment (§3.5) is Phase-3 Tone Engineer agent reasoning layered on top. Keeps the §3.7 two-sided guard clean (index structural, agent reasons on it). §3.6 example is read as illustrative, not a structural contract. | 2026-05 |
...
| 10 | Verification floor (`runSelfTests`/`?selftest`) + loud-guard over snapshot self-containment | 2026-06-06 review found zero automated checks and a save path that silently dropped the Gear/Dashboard tabs when a snapshot was moved from its sibling `.js` files. Added a zero-dependency self-test harness (behavior floor, respects LD#2/#5) + a `/session-close` commit gate. Chose a loud Dashboard load-guard over full self-containment: gear/dashboard data is static, git-backed (LD#6) so loss is recoverable, and runtime inlining is infeasible under file:// (no `fetch`; `dashboard-data.js` carries functions, not just data). Save filename now versioned + timestamped (forking guard). Only `PRESETS` is true in-app-mutated data; its marker-regex save path is unchanged. | 2026-06-06 |
| 9 | Gear specs are verifiability-stamped: pedal `power` block + per-item `specProvenance`; `default` is author-inferred, never spec | "100% verifiable" demand. Pedal entries had no power data and no provenance on their own control specs; `default` knob values read as spec but were author-invented. Per-item granularity chosen over per-field (gear specs trace to one manufacturer doc per item, unlike heterogeneous per-knob recipe sources). `default` retained for Tone Engineer convenience but explicitly excluded from the `specProvenance` claim. Notation (o'clock vs numeric) was already locked via §3.4 / Conflict 1 — not reopened. Additive schema only (durability rule). GEAR_TAB_DESIGN §3.1 + §9 amended. Population across all pedals is a tracked Phase effort, not yet done. | 2026-05 |
