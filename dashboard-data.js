...
const AGENT_DASHBOARD = {
  ...
  dimensions: [
    ...
    { label: 'Pentatonic as skeleton',
      ...
      drivers: ['Brewster overlay shows penta as rings (comfort zone)', 'Pentatonic view mode isolates the skeleton', 'Pathways frame expansions as "penta + N notes"'],
      limiters: ['EVH scores lower — intervallic expansion beyond penta not yet visualized as paths'],
      linkedLoops: ['arpeggio-view'],
      projected: '+0.8 when arpeggio-view shows chord-tone paths within penta positions',
    },
    { label: 'Chord-tone targeting',
      target: 10, current: 8.3, scores: { hendrix: 9, evh: 7, srv: 9 },
      targetDef: 'TONE shows which notes ARE the chord at every position, at playing speed',
      drivers: ['Play Mode target note logic correct', 'Overlay shows chord tones as solid dots', 'Forward vision shows next target'],
      limiters: ['EVH scores lower — no sequential arpeggio paths through chord tones', 'Decoder fretboard lacks classification'],
      linkedLoops: ['arpeggio-view', 'note-classify-decoder'],
      projected: '+1.0 when arpeggio paths + decoder classification done',
    },
    { label: 'Rhythm / lead integration',
      ...
      drivers: ['Brewster overlay shows chord shapes in position', 'ACE zone model maps triad voicings'],
      limiters: ['No triad chaining through progressions', 'No voicing-to-melody transition paths', 'This may be partially unsolvable through visualization — flagged by all agents'],
      linkedLoops: ['triad-chaining'],
      projected: '+1.5 when triad chaining implemented. Ceiling may be ~8 — dynamics/phrasing require the guitar in hand.',
    },
    { label: 'Physical instrument / position',
      target: 9, current: 5.3, scores: { hendrix: 5, evh: 5, srv: 6 },
      targetDef: 'TONE shows navigation in terms of fretboard geography — zones, positions, hand movement — not abstract theory',
      drivers: ['CAGED zone overlay exists (non-diatonic chords)', 'Root dot squares provide spatial anchoring', 'Between-fret dots aid position awareness'],
      limiters: ['CAGED zone not shown on diatonic chords (decided to add — trust Stevie)', 'No arpeggio paths showing hand movement through positions', 'No song route mapping showing position changes per section', 'Decoder fretboard is analysis-only, no position context'],
      linkedLoops: ['arpeggio-view', 'song-route', 'note-classify-decoder'],
      projected: '+1.5 from CAGED-on-diatonic + arpeggio paths. +2.5 if song routes added.',
    },
    { label: 'Modal extension visibility',
      target: 8, current: 4.0, scores: { hendrix: 3, evh: 4, srv: 5 },
      targetDef: 'Dorian, Mixolydian, and chromatic extensions are visible as departure options from the pentatonic default',
      drivers: ['PENTA_PATHWAYS_EXT data exists for Dorian + Blues', 'Insight cards describe extensions in pentatonic view'],
      limiters: ['Dorian 6th NOT visible as ghost in Play Mode', 'Chromatic approach notes filtered to diatonic only', 'Major penta tones over dominant chords not surfaced'],
      linkedLoops: ['dorian-ghost', 'chromatic-approach', 'penta-switch'],
      projected: '+2.0 from dorian ghost + chromatic approach. +3.0 if penta-switch added.',
    },
    { label: 'Forward vision / voice leading',
      target: 10, current: 9.0, scores: { hendrix: 9, evh: 9, srv: 9 },
      targetDef: 'TONE shows where the player is going NEXT — the upcoming chord\'s target and how to approach it',
      drivers: ['Next target diamonds (Play Mode)', 'Approach note triangles (voice leading)', 'Nav prompt shows next note + role'],
      limiters: ['Approach notes are diatonic-only — chromatic half-step excluded'],
      linkedLoops: ['chromatic-approach'],
      projected: '+0.5 when chromatic approach fixed. Near ceiling.',
    },
    ...
    { label: 'Chromatic vocabulary support',
      target: 8, current: 4.0, scores: { hendrix: 4, evh: 3, srv: 5 },
      targetDef: 'Chromatic passing tones, approach notes, and non-diatonic connections are visible as navigation options',
      drivers: ['7#9 chord type added (contains b3 + 3 tension)', 'Blues scale b5 pathway added'],
      limiters: ['Approach notes diatonic-only — the primary gap', 'No chromatic enclosure visualization', 'EVH-specific chromatic passing patterns not surfaced'],
      linkedLoops: ['chromatic-approach'],
      projected: '+2.0 from chromatic approach fix. Further gains require Layer 10 (Protocol Translator).',
    },
  ...
  features: [
    ...
    { name: 'Arpeggio view',               layer: 'L9',      status: 'gap' },
    { name: 'Triad chaining',              layer: 'L9',      status: 'gap' },
    { name: 'Song route mapping',           layer: 'L9',      status: 'gap' },
    { name: 'Note classification on Decoder fretboard', layer: 'L9', status: 'gap' },
